...
function enumWindows() {
    try {
        return require('./process-manager').enumRobloxWindows();
    }
    catch {
        return [];
    }
}
function matchRobloxPid(placeName, placeId) {
...
class WsServer {
    ...
    mount(server) {
        ...
        this.wss.on('connection', (ws) => {
            ...
            const cleanup = (isError) => {
                if (isError)
                    console.error('[WS]', isError);
            ...
            const finalizeReg = (pid, name, jobId, placeId) => {
            ...
            ws.on('close', () => cleanup());
            ws.on('error', (err) => cleanup(err?.message || 'Connection error'));
        });
        // Task dispatch
        try {
            this._taskHandler = (task) => {
                if (task.targetPid) {
                    const wid = this.pidMap.get(Number(task.targetPid));
                    if (!wid)
                        return console.log(`[WS] No worker for PID ${task.targetPid}`);
                    const info = this.workers.get(wid);
                    if (info)
                        this._safeSend(info.ws, { type: 'task', id: task.id, tool: task.type, args: task.args, timestamp: task.timestamp, pid: task.targetPid, workerId: wid });
                }
                else {
                    let n = 0;
                    for (const ws of this.allConnections)
                        if (this._safeSend(ws, { type: 'task', id: task.id, tool: task.type, args: task.args, timestamp: task.timestamp }))
                            n++;
                    if (n)
                        console.log(`[WS] Broadcast ${task.type} to ${n} connection(s)`);
                }
            };
            this.queue.on('task', this._taskHandler);
        }
        catch (e) {
