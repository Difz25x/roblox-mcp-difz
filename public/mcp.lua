...
local function resolvePath(str)
	...
	if srv then
		local o, e = pcall(function()
			return cloneref(game:GetService(srv))
		end)
		return o and e or nil
	end
	if clean:sub(1, 5) == "game." then
		clean = clean:sub(6)
	elseif clean:sub(1, 10) == "workspace." then
		clean = "Workspace." .. clean:sub(11)
	elseif clean:sub(1, 8) == "players." then
		clean = "Players." .. clean:sub(9)
	end
	local parts = {}
	...
	elseif parts[1] == "Players" then
		obj = Players
		table.remove(parts, 1)
	elseif parts[1] == "LocalPlayer" then
		obj = LocalPlayer
		table.remove(parts, 1)
	else
		obj = game
...
local function serialize(v, d)
	...
	if t == "CFrame" then
		local x, y, z, qx, qy, qz, qw = v:GetComponents()
	...
	if t == "Instance" then
		return { Name = v.Name, ClassName = v.ClassName, Path = getFullPath(v) }
	end
	return tostring(v)
end
...
pcall(function()
	local function testUncCapabilities()
		...
		pcall(function()
			local r = { _G.identifyexecutor() }
			if type(r[1]) == "string" then
				cap.executorName = r[1]
			end
		end)
		pcall(function()
			cap.hasGenv = (_G.getgenv() ~= nil)
		end)
...
local function wsReconnect()
	...
	if not connectFn then
		return false, "No WebSocket support"
	end
...
local function wsPoll()
	...
	if #WS_BUFFER > 0 then
		local m = table.remove(WS_BUFFER, 1)
		return { type = m.tool, id = m.id, args = m.args or {}, pid = m.pid }
	end
	return nil
end
...
local function wsSend(id, data, err, taskPid)
	...
	local ok, encoded = pcall(function()
		return HttpService:JSONEncode({ type = "result", id = id, data = data, error = err, pid = pid })
	end)
	if ok then
		...
		local errResponse = HttpService:JSONEncode({
			type = "result",
			id = id,
			data = { success = false },
			error = "Failed to serialize result: " .. tostring(encoded),
			pid = pid,
		})
		return pcall(function()
			WS:Send(errResponse)
...
local function handleGetMetadata(args)
	...
	if args.include_performance then
		data.FPS = 60
		data.Memory = collectgarbage("count")
	end
...
local function handleDumpPlayers(args)
	...
	for _, p in ipairs(Players:GetPlayers()) do
		...
		if p.Character then
			...
			if hum and args.include_character_humanoid then
				entry.Health = hum.Health
				entry.MaxHealth = hum.MaxHealth
				entry.WalkSpeed = hum.WalkSpeed
			end
			if args.include_backpack then
				local bp = p:FindFirstChild("Backpack")
				if bp then
					local items = {}
					for _, c in ipairs(bp:GetChildren()) do
						table.insert(items, { Name = c.Name, ClassName = c.ClassName })
					end
					entry.Backpack = items
				end
			end
			if args.include_proximity_prompts then
				local prompts = {}
				for _, c in ipairs(p.Character:GetDescendants()) do
					if c:IsA("ProximityPrompt") then
						table.insert(prompts, { Name = c.Name, Path = getFullPath(c) })
					end
				end
				entry.ProximityPrompts = prompts
			end
		end
		...
		if root then
			local function find(inst)
				if inst:IsA("RemoteEvent") or inst:IsA("RemoteFunction") or inst:IsA("UnreliableRemoteEvent") then
					table.insert(remotes, { Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst) })
				end
...
local function handleConsoleLog(args)
	local lt = args.log_type or "all"
	local ml = args.max_lines or 100
	...
	for i = #logs, 1, -1 do
		...
		local e = logs[i]
		local mt = tostring(e.messageType):lower()
		if lt == "all" or mt:find(lt) then
			table.insert(entries, 1, { message = e.message, type = mt, timestamp = e.timestamp or tick() })
			count = count + 1
		end
	end
...
local function handleWorkspaceObjects(args)
	...
	local fc = args.class_filter or ""
	local results = {}
	local function walk(inst, d)
		if mr > 0 and #results >= mr then
			return
		end
		if d > md then
			return
		...
		if d > 0 then
			if fc == "" or inst.ClassName == fc then
				table.insert(results, { Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst) })
			end
		end
		for _, c in ipairs(inst:GetChildren()) do
			walk(c, d + 1)
...
local function handlePlayerState()
	if not LocalPlayer then
		return { success = false, error = "No LocalPlayer" }
	end
...
local function handleRemoteFire(args)
	local rp = args.remote_path or ""
	if rp == "" then
		return { success = false, error = "remote_path required" }
	end
	local inst, err = resolvePath(rp)
	if not inst then
		return { success = false, error = err }
	end
	local fa = jsonDecode(args.arguments) or args.args or {}
	local method = args.method or "FireServer"
	local ok, r = pcall(function()
		if method == "FireServer" then
			inst:FireServer(table.unpack(fa))
			return { fired = true }
		else
			return inst:InvokeServer(table.unpack(fa))
		end
	end)
	if not ok then
		return { success = false, error = "Failed: " .. tostring(r) }
	end
	return { success = true, result = serialize(r) }
end
local function handleRemoteConns(args)
	...
	local conns = getconnections(inst)
	local results = {}
	for _, c in ipairs(conns) do
		table.insert(results, { connected = c.Connected, enabled = c.Enabled, functionInfo = tostring(c.Function) })
	end
...
local function handleTreeExplore(args)
	...
	if action == "walk" then
		...
		local md = args.max_depth or 10
		local mr = args.max_results or 100
		local fc = args.class_filter or ""
		local np = args.name_pattern or ""
		local results = {}
		local function walk(inst, d)
			if mr > 0 and #results >= mr then
				return
			end
			if md >= 0 and d > md then
				return
			end
			if d > 0 then
				local match = true
				if fc ~= "" then
					local cls = {}
					for c in fc:gmatch("[^,]+") do
						cls[c:match("^%s*(.-)%s*$") or ""] = true
					end
					if next(cls) and not cls[inst.ClassName] then
						match = false
					end
				end
				if match and np ~= "" then
					match = inst.Name:lower():find(np:lower()) ~= nil
				end
				if match then
					table.insert(results, { Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst) })
				end
	...
	if action == "services" then
		...
		for _, c in ipairs(game:GetChildren()) do
			if c.ClassName:find("Service") or c.ClassName:find("Provider") then
				table.insert(results, { Name = c.Name, ClassName = c.ClassName, Path = getFullPath(c) })
			end
		end
		return { success = true, count = #results, services = results }
	end
	if action == "children" then
		local inst, err = resolvePath(args.instance_path or "game")
		if not inst then
			return { success = false, error = err }
		end
		local children = {}
	...
	if action == "path_resolve" then
		local inst = resolvePath(args.path or "")
		if not inst then
	...
	if action == "subtree" then
		...
		local function walk(inst, d)
			...
			local e = { Name = inst.Name, ClassName = inst.ClassName, children = {} }
			for _, c in ipairs(inst:GetChildren()) do
				local s = walk(c, d + 1)
				if s then
					table.insert(e.children, s)
				end
			end
			if d > 0 or e.Name ~= "game" then
				return e
			end
			results = e.children
			return nil
		...
		walk(root, 0)
		return { success = true, tree = results }
	end
	if action == "proximity" then
		local pos = args.position or { x = 0, y = 0, z = 0 }
		local radius = args.radius or 50
		local root, err = resolvePath(args.scope or "game.Workspace")
		if not root then
			return { success = false, error = err }
		end
		local mr = args.max_results or 200
		local results = {}
		local function walk(inst)
			...
			if ok then
				local dx = cf.Position.X - pos.x
				local dy = cf.Position.Y - pos.y
				local dz = cf.Position.Z - pos.z
				local dist = math.sqrt(dx * dx + dy * dy + dz * dz)
				if dist <= radius then
					table.insert(
						results,
						{ Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst), distance = dist }
					)
				end
			end
			for _, c in ipairs(inst:GetChildren()) do
	...
	if action == "tag_collect" then
		local tn = args.tag_name or ""
		if tn == "" then
			return { success = false, error = "tag_name required" }
		end
		local ok, instances = pcall(function()
			return CollectionService:GetTagged(tn)
		end)
		if not ok then
			return { success = false, error = "CollectionService error" }
		end
		local mr = args.max_results or 200
		local results = {}
		local c = 0
		for _, inst in ipairs(instances) do
			if mr > 0 and c >= mr then
				break
			end
			table.insert(results, { Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst) })
			c = c + 1
	...
	if action == "attribute_seek" then
		local an = args.attribute_name or ""
		local av = args.attribute_value
		local root, err = resolvePath(args.scope or "game")
		if not root then
			return { success = false, error = err }
		end
		local mr = args.max_results or 200
		...
		local function walk(inst)
			...
			local ha, val = pcall(function()
				return inst:GetAttribute(an)
			end)
			if ha and val ~= nil and (av == nil or val == av) then
				table.insert(results, { Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst) })
			end
			for _, c in ipairs(inst:GetChildren()) do
				walk(c)
	...
	if action == "clickdetector" then
		...
		if target == "" then
			return { success = false, error = "target_path required" }
		end
		...
		if not cd then
			return { success = false, error = "No ClickDetector" }
		end
		fireclickdetector(cd)
		return { success = true, message = "Fired ClickDetector" }
	end
	if action == "proximity_prompt" then
		...
		if not pp then
			return { success = false, error = "No ProximityPrompt" }
		end
		fireproximityprompt(pp)
		return { success = true, message = "Fired ProximityPrompt" }
	end
	if action == "nil_realm" then
		local fc = args.filter_by_class or ""
		local mi = args.max_instances or 200
		local results = {}
		for _, c in ipairs(getnilinstances()) do
			if mi > 0 and #results >= mi then
				break
			end
			if fc == "" or c.ClassName == fc then
				table.insert(results, { Name = c.Name, ClassName = c.ClassName, Path = getFullPath(c) })
			end
		end
		return { success = true, count = #results, instances = results }
...
local function handlePropertyRead(args)
	local inst, err = resolvePath(args.instance_path or args.path or "game")
	if not inst then
		return { success = false, error = err }
	end
	local pn = (args.property_map or args.property_list or args.properties) or { "Name", "ClassName" }
	local result = { _path = getFullPath(inst), _className = inst.ClassName }
	for _, p in ipairs(pn) do
		local ok, v = pcall(function()
			return inst[p]
		end)
		if ok then
			result[p] = serialize(v)
		end
	end
	return { success = true, data = result }
end
local function handleGuiInject(args)
	local gt = args.gui_type or "ScreenGui"
	local gn = args.gui_name or "McpOverlay"
	local pp = args.parent_path or 'game:GetService("CoreGui")'
	local parent, err = resolvePath(pp)
	if not parent then
		return { success = false, error = "Parent not found" }
	end
	local props = jsonDecode((args.property_map or args.property_list or args.properties)) or {}
	local ok, ng = pcall(function()
		local inst = Instance.new(gt)
		inst.Name = gn
	...
	if not ok then
		return { success = false, error = "GUI create failed: " .. tostring(ng) }
	end
...
local function handleGuiDump(args)
	...
	local function walk(inst, d)
		...
		local e = { Name = inst.Name, ClassName = inst.ClassName }
		if
			inst.ClassName:find("Gui")
			or inst.ClassName:find("Button")
			or inst.ClassName:find("Label")
			or inst.ClassName:find("Box")
		then
			for _, p in ipairs({ "Position", "Size", "Text", "TextColor3", "BackgroundColor3", "Visible" }) do
				local ok, v = pcall(function()
					return inst[p]
				end)
				if ok then
		...
		e.children = {}
		for _, c in ipairs(inst:GetChildren()) do
			table.insert(e.children, walk(c, d + 1))
		end
		return e
	end
	for _, c in ipairs(root:GetChildren()) do
		table.insert(results, walk(c, 0))
	end
	return { success = true, guiTree = results }
end
local function handleFileOp(args)
	local a = args.action or "read"
	...
	if a == "read" then
		...
		if not ok then
			return { success = false, error = "readfile failed" }
		end
		return { success = true, path = p, content = c, size = #c }
	end
	if a == "write" then
		local c = args.content or ""
		if p == "" then
			return { success = false, error = "path required" }
		end
		pcall(writefile, p, c)
		return { success = true, path = p, bytes = #c }
	end
	if a == "delete" then
		if p == "" then
			return { success = false, error = "path required" }
	...
	if a == "list" then
		local ok, files = pcall(listfiles, p)
		if not ok then
			return { success = false, error = "listfiles failed" }
		end
		local r = {}
		for _, f in ipairs(files) do
			table.insert(r, { name = f, isFile = pcall(isfile, f) and isfile(f) or false })
		end
		return { success = true, count = #r, files = r }
	end
	return { success = false, error = "Unknown action" }
end
local function handleHiddenProp(args)
	local a = args.action or "get"
	local prop = args.property or ""
	local path = args.instance_path or ""
	if prop == "" then
	...
	if a == "get" then
		...
		if not ok then
			return { success = false, error = "Cannot read hidden property" }
		end
		return { success = true, property = prop, value = serialize(val) }
	end
	if a == "set" then
		local val = args.value
		local ok = pcall(sethiddenproperty, inst, prop, val)
		if not ok then
			return { success = false, error = "Cannot set hidden property" }
		end
		return { success = true, property = prop }
	end
	if a == "make_scriptable" then
...
local function handleStateBypass(args)
	...
	if a == "speed" then
		hum.WalkSpeed = args.walk_speed or 50
		return { success = true }
	end
	if a == "jump" then
		hum.JumpPower = args.jump_power or 100
		return { success = true }
	end
	if a == "health" then
		if args.mode == "heal_to_full" then
			hum.Health = hum.MaxHealth
		else
			hum.Health = args.health_value or hum.MaxHealth
		end
		return { success = true }
	end
	if a == "teleport" then
		...
		local c = args.coordinates or { x = 0, y = 0, z = 0 }
		hrp.CFrame = CFrame.new(Vector3.new(c.x, c.y, c.z))
		return { success = true }
	end
	return { success = false, error = "Unknown action" }
end
local function handleInputSim(args)
	...
	if a == "key_press" then
		...
		if VirtualInputManager then
			pcall(function()
				VirtualInputManager:SendKeyEvent(true, vk, false, game)
				task.wait(0.03)
				VirtualInputManager:SendKeyEvent(false, vk, false, game)
			end)
		end
...
local function handleSandboxExec(args)
	...
	local ok2, r = pcall(fn)
	if not ok2 then
		return { success = false, error = "Runtime: " .. tostring(r) }
	end
	return { success = true, result = serialize(r) }
end
...
local function handleGetLoadedModules(args)
	...
	if not ok then
		return { success = false, error = "getloadedmodules not supported" }
	end
...
local function handleRunningScripts(args)
	...
	if not ok then
		return { success = false, error = "getrunningscripts not supported" }
	end
...
local function handleScriptClosure(args)
	...
	if not ok then
		return { success = false, error = "getscriptclosure not supported" }
	end
...
local function handleScriptHash(args)
	...
	if not ok then
		return { success = false, error = "getscripthash not supported" }
	end
...
local function handleCallingScript()
	...
	if not ok then
		return { success = false, error = "getcallingscript not supported" }
	end
	if not s then
		return { success = true, script = nil }
	end
...
local function handleScriptEnv(args)
	...
	if not ok then
		return { success = false, error = "getsenv not supported or script not running" }
	end
	local keys = {}
	local maxK = args.max_keys or 50
	local count = 0
...
local function handleRobloxEnv()
	...
	if not ok then
		return { success = false, error = "getrenv not supported" }
	end
	local keys = {}
	local count = 0
	for k, v in pairs(env) do
...
local function handleSandboxAnalysis(args)
	...
	pcall(function()
		isExecutor = isexecutorclosure(function() end)
	end)
	local caps = {
...
local function handleMetatableSeer(args)
	...
	if not mt then
		return { success = true, hasMetatable = false }
	end
	local methods = args.metamethods
		or {
			"__index",
			"__newindex",
			"__call",
...
local function handleMetatableModifier(args)
	...
	if action == "set_raw" then
		if not args.new_metatable then
			return { success = false, error = "new_metatable required" }
		end
...
local function handleFuncInterceptor(args)
	...
	if not target then
		return { success = false, error = "Cannot resolve: " .. funcPath }
	end
	...
	if action == "install" and hookFn then
		...
		if not ok4 then
			return { success = false, error = "hookfunction not supported: " .. tostring(orig) }
		end
...
local function handleClosureType(args)
	...
	if not ok or type(fn) ~= "function" then
		...
		if inst then
			...
			if not ok2 then
				return { success = false, error = "Cannot resolve to a closure" }
			end
			fn = sc
		else
			return { success = false, error = "Cannot resolve: " .. path }
		end
	end
	local ok3, resolved = pcall(fn)
	if not ok3 then
		return { success = false, error = "Cannot evaluate: " .. path }
	end
	local result = { type = typeof(resolved) }
	pcall(function()
		result.isCClosure = iscclosure(resolved)
	end)
	pcall(function()
		result.isLClosure = islclosure(resolved)
	end)
	pcall(function()
		result.isExecutorClosure = isexecutorclosure(resolved)
	end)
...
local function handleRegistryScan(args)
	...
	if not ok then
		return { success = false, error = "getreg not supported" }
	end
	local results = {}
	local count = 0
...
local function handleClosureInspect(args)
	...
	local ok2, resolved = pcall(fn)
	if not ok2 or type(resolved) ~= "function" then
...
local function handleInstanceComparer(args)
	local a = args.instance_a or ""
	local b = args.instance_b or ""
	if a == "" or b == "" then
		return { success = false, error = "instance_a and instance_b required" }
	end
	local instA = resolvePath(a)
	local instB = resolvePath(b)
	if not instA then
		return { success = false, error = "Cannot resolve: " .. a }
	end
	if not instB then
		return { success = false, error = "Cannot resolve: " .. b }
	end
	local same = false
	pcall(function()
		same = compareinstances(instA, instB)
	end)
	if same == nil then
		same = (instA == instB)
	end
	return {
		success = true,
		instanceA = { Name = instA.Name, Path = getFullPath(instA) },
		instanceB = { Name = instB.Name, Path = getFullPath(instB) },
		sameReference = same,
		sameClass = instA.ClassName == instB.ClassName,
	}
end
...
local function handleSiblingEnum(args)
	...
	if not parent then
		return { success = true, siblings = {}, count = 0, message = "Root object, no parent" }
	end
	local results = {}
...
local function handlePropertySeeker(args)
	...
	if propName == "" then
		return { success = false, error = "property_name required" }
	end
	...
	local function walk(inst)
		...
		if ok and (propValue == nil or val == propValue) then
			table.insert(
				results,
				{ Name = inst.Name, ClassName = inst.ClassName, Path = getFullPath(inst), value = tostring(val) }
			)
			count = count + 1
		end
		for _, c in ipairs(inst:GetChildren()) do
...
local function handleHumanoidState(args)
	...
	return {
		...
		sit = hum.Sit,
		floorMaterial = tostring(hum.FloorMaterial),
	}
end
...
local function handleMouseButton(args)
	...
	if args.button == "right" then
		button = Enum.UserInputType.MouseButton2
	elseif args.button == "middle" then
...
local function handleScrollWheel(args)
	local scrollX = args.scroll_x or 0
	local scrollY = args.scroll_y or 0
	if VirtualInputManager then
		pcall(function()
			VirtualInputManager:SendScrollWheelEvent(0, 0, scrollX, scrollY, game)
		end)
		return { success = true }
	end
	return { success = false, error = "VirtualInputManager not available" }
...
local function handleCameraControl(args)
	...
	if not workspaceCam then
		return { success = false, error = "No camera" }
	end
	if action == "get" then
		return {
			success = true,
			cframe = serialize(workspaceCam.CFrame),
			focus = serialize(workspaceCam.Focus),
			fieldOfView = workspaceCam.FieldOfView,
			cameraType = tostring(workspaceCam.CameraType),
		}
	end
...
local function handleNotificationHide()
	pcall(function()
		for _, c in ipairs(gethui():GetDescendants()) do
			if c:IsA("Notification") or c.Name:find("Notification") then
				c:Destroy()
			end
		end
	end)
...
local function handleESP(args)
	...
	if state == nil then
		state = (action == "enable" or action == "toggle")
	end
	return {
		success = true,
		message = "ESP requires Drawing lib, check executor support",
		action = action,
		class = targetClass,
		enabled = state,
	}
...
local function handleUiChangeWatcher(args)
	...
	local function matchFilter(inst)
		if filterClass ~= "" and inst.ClassName ~= filterClass then
			return false
		end
		if filterName ~= "" and not inst.Name:lower():find(filterName, 1, true) then
			return false
		end
		return true
	end
	local function shouldTrack(ct)
		if #changeTypes == 0 then
			return true
		end
		for _, t in ipairs(changeTypes) do
			if t == ct then
				return true
			end
		end
	...
	if shouldTrack("child_added") then
		table.insert(
			...
			root.DescendantAdded:Connect(function(inst)
				if not watching or #changes >= maxChanges then
					return
				end
	...
	if
		shouldTrack("property_changed")
		or shouldTrack("visibility_changed")
		or shouldTrack("position_changed")
		or shouldTrack("size_changed")
	then
		local props = {}
		if shouldTrack("visibility_changed") then
			table.insert(props, "Visible")
		end
		if shouldTrack("position_changed") then
			table.insert(props, "Position")
		end
		if shouldTrack("size_changed") then
			table.insert(props, "Size")
		end
		if shouldTrack("property_changed") then
			for _, p in ipairs(propFilters) do
			...
			if inst:IsA("GuiObject") then
				for _, pn in ipairs(props) do
					local ok, sig = pcall(function()
						return inst:GetPropertyChangedSignal(pn)
					end)
					if ok then
						table.insert(
							propConns,
		...
		for _, c in ipairs(propConns) do
	...
	return {
		...
		changes = changes,
		changeCount = #changes,
		watchedSeconds = tick() - startTime,
		reachedMax = (#changes >= maxChanges),
		timedOut = (#changes < maxChanges),
	}
end

local function handleDisableAntiCheat(args)
	local hookKick = args.hook_kick
	if hookKick == nil then
		hookKick = true
	end
	local disableScripts = args.disable_ac_scripts
	if disableScripts == nil then
	...
	if afkInterval == nil or afkInterval < 5 then
		afkInterval = 30
	end
		...
		pcall(function()
			...
			if TeleportService then
				local teleportMethods =
					{ "Teleport", "TeleportToPlaceInstance", "TeleportToPlaceEndpointAsync", "TeleportAsync" }
...
local HANDLERS = {
	...
	world_to_screen_converter = function(a)
		...
		local p = Vector3.new(a.world_x or 0, a.world_y or 0, a.world_z or 0)
	...
	instance_duplicator = function(a)
		...
		local parent = resolvePath(a.parent_path or "")
		if parent then
			clone.Parent = parent
		end
...
proxyToServer = function(toolName, args)
	return {
		...
		error = "Feature '"
			.. tostring(toolName)
			.. "' is not handled by this executor. The executor's HttpService:PostAsync is blocked and no local handler was registered.",
		tool = toolName,
	}
end
